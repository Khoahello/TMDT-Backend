...
def get_total_revenue(start_date, end_date, user_id, role_name):
    ...
    try:
        ...
        base_sql = """
            ...
            FROM Orders o
            WHERE o.OrderDate::date >= %s::date AND o.OrderDate::date <= %s::date
        """
        ...
        return False, "Định dạng ngày tháng không hợp lệ (Chuẩn: YYYY-MM-DD)", None
...
def get_top_products(limit=10, start_date=None, end_date=None, user_id=None, role_name=None):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        
        # [CHUẨN JSON]: Ép p.ProductID::text
        base_sql = """
            SELECT 
                p.ProductID::text AS "ProductID", p.ProductName AS "ProductName", c.CategoryName AS "CategoryName",
        ...
        if start_date and end_date:
            base_sql += " AND o.OrderDate::date >= %s::date AND o.OrderDate::date <= %s::date "
        ...
        if conn: conn.rollback()
        return False, "Định dạng khoảng thời gian không hợp lệ", None
    except Exception as e:
        if conn: conn.rollback()
        return False, str(e), None
    finally:
...
def get_order_status_breakdown(user_id, role_name):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
def get_revenue_by_shop(start_date, end_date, user_id, role_name):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        
        # [CHUẨN JSON]: Ép s.ShopID::text
        base_sql = """
            SELECT 
                s.ShopID::text AS "ShopID", s.ShopName AS "ShopName", COALESCE(SUM(o.TotalAmount), 0) AS "Revenue"
            FROM Shops s
            LEFT JOIN Orders o ON s.ShopID = o.ShopID AND o.Status != 'Đã hủy' AND o.OrderDate::date >= %s::date AND o.OrderDate::date <= %s::date
            WHERE 1=1
        """
        params = [start_date, end_date]
        if role_name == 'Manager':
        ...
        if conn: conn.rollback()
        return False, "Định dạng ngày tháng không hợp lệ", None
    except Exception as e:
        if conn: conn.rollback()
        return False, str(e), None
    finally:
...
def get_revenue_by_category(start_date, end_date, user_id, role_name):
    conn = get_db_connection()
    if not conn: return False, "Lỗi kết nối", None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        
        # [CHUẨN JSON]: Ép c.CategoryID::text
        base_sql = """
            SELECT 
                c.CategoryID::text AS "CategoryID", c.CategoryName AS "CategoryName", COALESCE(SUM(od.Quantity * od.UnitPrice), 0) AS "Revenue"
            FROM Categories c
            JOIN Products p ON c.CategoryID = p.CategoryID 
            JOIN OrderDetails od ON p.ProductID = od.ProductID 
            JOIN Orders o ON od.OrderID = o.OrderID
            WHERE o.Status != 'Đã hủy' AND o.OrderDate::date >= %s::date AND o.OrderDate::date <= %s::date
        """
        params = [start_date, end_date]
        if role_name == 'Manager':
            base_sql += " AND o.ShopID IN (SELECT ShopID FROM Shops WHERE ManagerID = %s) "
